...
from surprise.model_selection import cross_validate
import pickle
df_ratings = pd.read_csv('mlratings.csv', sep='::', names=['userId', 'movieId', 'rating', 'timestamp'],engine = 'python', encoding = 'latin-1')
df_movies = pd.read_csv('movies.csv', sep='::', names=['movieId', 'title', 'genres'],engine = 'python', encoding = 'latin-1')
